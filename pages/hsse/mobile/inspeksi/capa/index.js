...
const DetailInspeksi = ({ data, error, token, sessionData }) => {
  const formik = useFormik({
    initialValues: {
      id: data?.id || 0,
      nomorInspeksi: data?.nomorInspeksi || "",
      ...
      tglPengecekanTerakhir: data?.tglPengecekanTerakhir || "",
      tglPengecekanSelanjutnya: data?.tglPengecekanSelanjutnya || "",
      status: data?.status || "",
      detailJenis: data?.detailJenis || [],
      detailApar: data?.detailApar || [],
      parameterInspeksi: data?.parameterInspeksi || [],
      ...
      parameterInspeksi: yup
        ...
        .of(
          yup.object().shape({
            ...
            tindakLanjutCAPA: yup.string().when("statusIsOk", {
              is: false,
              then: yup.string().required("Harus diisi"),
              otherwise: yup.string(),
            }),
            keteranganCAPA: yup.string().when("statusIsOk", {
              is: false,
              then: yup.string().required("Harus diisi"),
              otherwise: yup.string(),
            }),
            lampiranParameterInspeksi: yup.array().min(1, "Harus diisi"),
            lampiranCAPAParameterInspeksi: yup.array().when("statusIsOk", {
              is: false,
              then: yup.array().min(1, "Harus diisi"),
              otherwise: yup.array(),
            }),
          })
        )
        ...
        const newData = await Promise.all(
          values.parameterInspeksi.map(async (param) => {
            const newLampiranParameterInspeksi = await Promise.all(
              param.lampiranCAPAParameterInspeksi.map(async (file) => {
                if (file.photo) {
            ...
            return {
              ...param,
              lampiranCAPAParameterInspeksi: newLampiranParameterInspeksi,
            };
          })
        );
  ...
  const SubmitHandler = async (newData) => {
    ...
    const hitSubmit = dispatch(
      submitCAPAInspeksi(compId, role, upn, name, email, newData)
    )
      .then((res) => {
        setLoading(false);
  ...
  const alertNotification = () => {
    confirmAlertNotification(
      "Konfirmasi submit inspeksi CAPA",
      "Apakah anda yakin submit inspeksi CAPA?",
      () => {
        formik.submitForm();
      },
      () => {}
      ...
      {error ? (
        ...
        <div className="">
          ...
          <div className="mt-1 mx-2">
            <ParameterContainer formik={formik} isCAPA={true} />
          </div>
          <div className="m-2">
            <Button
              color="primary mb-2"
...
export const getServerSideProps = wrapper.getServerSideProps(
  (store) => async (ctx) => {
    ...
    try {
      ...
      const nomorPeralatan = query.nomorPeralatan;
      const response = await store.dispatch(
        getInspeksiCapa(nomorPeralatan, {
